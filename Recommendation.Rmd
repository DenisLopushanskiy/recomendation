...
Построим рекомендательную модель по методу UBCF
```{r}
recom_model <- Recommender(data = recc_data_train, method = "UBCF",
parameter = list(nn = 30))
recom_model
```

```{r}
recom_predicted <- predict(object = recom_model, newdata = recc_data_test, n = 7)
recom_predicted
```
```{r}
str(recom_predicted)
...
Рекомендации для первого пользователя
```{r}
recom_user_1 <- recom_predicted@items[[1]]
recom_user_1
movies_user_1 <- recom_predicted@itemLabels[recom_user_1]
movies_user_1
```
